...
def make_excel(ifile, ofile, scenario):
	...
	for sector in sorted(df_capacity['sector'].unique()):
		...
		df_capacity_sector.to_csv(ofile + '_capacity_' + sector + '.csv', index=False, encoding='utf-8')
	...
	for sector in sorted(df_activity['sector'].unique()):
		...
		df_activity_sector.to_csv(ofile + '_activity_' + sector + '.csv', index=False, encoding='utf-8')
	...
	df_emissions.to_csv(ofile + '_emissions.csv', index=False, encoding='utf-8')
	...
	df_costs.to_csv(ofile + '_costs.csv', index=False, encoding='utf-8')
	...
	#print('Hello from Line 125')
	#_results
	#_results.to_csv('df_IamDataFrame_export_from_line_126_DB_to_Excel.csv', index=False)
